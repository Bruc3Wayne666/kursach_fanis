...
export default function FollowListScreen({ route, navigation }: any) {
    ...
    const handleFollow = async (targetUserId: string, isCurrentlyFollowing: boolean) => {
        ...
        try {
            // 🔥 РАЗНАЯ ЛОГИКА ДЛЯ ПОДПИСЧИКОВ И ПОДПИСОК
            if (type === 'followers') {
                // В подписчиках просто меняем статус подписки
                ...
                // 🔥 В ПОДПИСКАХ УДАЛЯЕМ ПОЛЬЗОВАТЕЛЯ ИЗ СПИСКА ПОСЛЕ ОТПИСКИ
                if (isCurrentlyFollowing) {
                    // Удаляем пользователя из списка
                    const updatedUsers = users.filter(user => user.id !== targetUserId);
                    setUsers(updatedUsers);
                } else {
                    // Это не должно происходить в подписках, но на всякий случай
                    const updatedUsers = users.map(user =>
                        user.id === targetUserId
            ...
            if (type === 'followers') {
                ...
                // В подписках перезагружаем список
                loadUsers();
            }
            ...
            <View style={styles.userCard}>
                ...
                {showButton && buttonText && (
                    <TouchableOpacity
                        ...
                        onPress={() => handleFollow(item.id, item.isFollowing || type === 'following')}
                        disabled={isLoading}
                    >
                        {isLoading ? (
